...
const ClientProfileHeader: React.FC<ClientProfileHeaderProps> = ({
  ...
  const locationDisplay =
    [profile.city, profile.country_name].filter(Boolean).join(", ") ||
    profile.location;

  // Determine online status
  const isOnline =
    lastActive && new Date(lastActive) > new Date(Date.now() - 15 * 60 * 1000);
      ...
      <div className="px-4 sm:px-6 pb-6">
        ...
        <div className="flex flex-col sm:flex-row items-start gap-4 -mt-12 sm:-mt-16">
          <div className="relative my-[10px]">
            ...
            <div
              className={`absolute bottom-1 right-1 w-5 h-5 rounded-full border-2 border-background ${isOnline ? "bg-green-500" : "bg-muted-foreground"}`}
            />
          </div>
            ...
            <div className="flex flex-wrap gap-2 mt-3">
              ...
              {profile.website && (
                ...
                >
                  ...
                  <span>{profile.website.replaceAll(/^https?:\/\//, "")}</span>
                </a>
              )}
