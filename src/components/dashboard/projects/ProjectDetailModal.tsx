...
const ProjectDetailModal = ({
      ...
      setMessages(
        (messagesData || []).map((m) => ({
          ...
          sender_name: m.sender_id
            ? senderProfiles[m.sender_id]?.full_name || "Unknown"
            : "Unknown",
          sender_avatar: m.sender_id
            ? senderProfiles[m.sender_id]?.avatar_url || undefined
            : undefined,
        })),
      );
    } catch (err) {
  ...
  const fetchProjectData = useCallback(
    async (signal?: AbortSignal, silent = false) => {
      ...
      try {
        ...
        if (projectError) {
          if (
            projectError.name === "AbortError" ||
            (projectError as any).code === "ABORT"
          )
            return;
          throw projectError;
        }
      ...
      .on(
        ...
        (payload) => {
          ...
          if ((payload.new as any)?.status === "success") {
            toast.success(
              "Payment confirmed! Your project is being updated...",
            );
            fetchProjectData(undefined, false);
          } else {
            fetchProjectData(undefined, false);
  ...
  const handleAddMilestone = async () => {
    ...
    try {
      const amountLocal = newMilestone.amount
        ? Number.parseFloat(newMilestone.amount)
        : 0;
      const rate = exchangeRates[userCurrency] || 1;
      const amountUSD =
        userCurrency === "USD"
          ? amountLocal
          : Number.parseFloat((amountLocal / rate).toFixed(8));
      ...
      if (existingMilestoneCols.includes("amount_usd"))
        milestoneInsert.amount_usd = amountUSD;
      if (existingMilestoneCols.includes("currency"))
        milestoneInsert.currency = userCurrency;
      if (existingMilestoneCols.includes("exchange_rate"))
        milestoneInsert.exchange_rate = rate;
      ...
      // Auto-sync the project budget summing all milestones
      const totalAmountUsd = milestones.reduce((sum, m) => sum + (m.amount || 0), 0) + amountUSD;
      await supabase.from("projects").update({ budget: totalAmountUsd, amount_usd: totalAmountUsd }).eq("id", projectId);
  ...
  const handleDeleteMilestone = async (milestoneId: string) => {
    try {
      ...
      const milestoneAmount = milestones.find(m => m.id === milestoneId)?.amount || 0;
      const newTotal = Math.max(0, milestones.reduce((sum, m) => sum + (m.amount || 0), 0) - milestoneAmount);
      await supabase.from("projects").update({ budget: newTotal, amount_usd: newTotal }).eq("id", projectId);
  ...
  const milestoneProgress =
    milestones.length > 0
      ...
      : 0;
  const progress = Math.max(project?.progress || 0, milestoneProgress);
          ...
          <div className="flex flex-col items-center justify-center py-24 gap-6">
            ...
            <div className="space-y-2 text-center">
              ...
              <p className="text-sm text-muted-foreground animate-pulse">
                Syncing latest updates from the cloud
              </p>
            </div>
          </div>
        </DialogContent>
      ...
      {!project ? (
        ...
        <DialogContent className="max-w-none w-screen h-screen max-h-none overflow-hidden flex flex-col p-0 pt-[var(--safe-top)] pb-[var(--safe-bottom)] pl-[var(--safe-left)] pr-[var(--safe-right)] gap-0 border-none shadow-none bg-background backdrop-blur-2xl rounded-none">
          <DialogHeader className="sr-only">
            ...
            <DialogDescription>
              Project details and collaboration workspace
            </DialogDescription>
          </DialogHeader>
            ...
            <div className="flex flex-col min-h-full max-w-7xl mx-auto w-full relative border-x border-border/5">
                ...
                <div className="relative z-10 flex flex-col sm:flex-row sm:items-end justify-between gap-8">
                  <div className="space-y-4 flex-1">
                    <div className="flex items-center gap-3">
                      ...
                      <Badge
                        variant="outline"
                        className="px-3 py-1 rounded-full bg-background/50 backdrop-blur-md border-primary/20 text-primary font-bold tracking-wide uppercase text-[10px]"
                      >
                    ...
                    <div className="space-y-1 sm:space-y-2">
                      ...
                      <div className="flex items-center gap-4 flex-wrap">
                        ...
                        <Link
                          to={
                            user?.id === project.artist_id
                              ? `/profile/${project.client_id}`
                              : `/artist/${project.artist_id}`
                          }
              ...
              <div className="px-6 sm:px-12 pb-12 space-y-12">
                ...
                <div className="space-y-4">
                  <div className="flex items-center gap-3">
                    ...
                    <h4 className="text-[11px] font-black uppercase tracking-[0.3em] text-muted-foreground/60">
                      Project Overview
                    </h4>
                  </div>
                  <div className="group relative p-6 sm:p-8 rounded-[2rem] sm:rounded-[3rem] bg-muted/10 border border-border/30 hover:bg-muted/20 transition-all duration-500">
                    ...
                    <p className="text-base sm:text-lg leading-relaxed text-foreground/80 whitespace-pre-wrap font-medium pl-4">
                      {project.description ||
                        "No description provided for this project. Use the communication tab to discuss requirements with your collaborator."}
                    </p>
                  </div>
                </div>
              </div>
              ...
              >
                  ...
                  <div className="relative group/tabs">
                    ...
                    <TabsList className="w-full h-auto min-h-[52px] sm:min-h-0 p-1.5 sm:p-2 bg-muted/50 rounded-[2rem] border border-border/40 flex items-stretch gap-1.5 sm:gap-2 overflow-x-auto no-scrollbar scroll-smooth shadow-inner">
                      ...
                      >
                        ...
                        <span className="font-black tracking-tight uppercase text-[9px] sm:text-[11px]">
                          Workflow
                        </span>
                      </TabsTrigger>
                      <TabsTrigger
                      ...
                      >
                        ...
                        <Badge
                          variant="secondary"
                          className="px-1 sm:px-2 py-0 h-3.5 sm:h-5 min-w-[14px] sm:min-w-[20px] text-[7px] sm:text-[10px] rounded-full bg-primary/10 text-primary border-none font-black"
                        >
                          {files.length}
                        </Badge>
                      </TabsTrigger>
                      ...
                      >
                        ...
                        <span className="font-black tracking-tight uppercase text-[9px] sm:text-[11px] whitespace-nowrap">
                          Chat
                        </span>
                        {messages.length > 0 && (
                ...
                <div className="space-y-12 pb-12">
                  <TabsContent
                    id="project-tab-content-workflow"
                    value="workflow"
                      ...
                      <div className="flex items-center gap-3 mb-8">
                        ...
                        <div>
                          ...
                          <p className="text-sm text-muted-foreground">
                            Track real-time progress and phase completion.
                          </p>
                        </div>
                      </div>
                  ...
                  <TabsContent
                    id="project-tab-content-milestones"
                    value="milestones"
                    className="mt-0 outline-none focus-visible:ring-0"
                  >
                      ...
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-sm text-muted-foreground">
                              Manage deliverables and payment phases.
                            </p>
                          </div>
                        </div>
                      </div>
                      ...
                      {milestones.length === 0 ? (
                        <div className="flex flex-col items-center justify-center py-20 text-center space-y-6 rounded-[2.5rem] border-2 border-dashed border-border/40 bg-muted/5 transition-all hover:bg-muted/10">
                          ...
                          <div className="space-y-2 max-w-xs">
                            ...
                            <p className="text-sm text-muted-foreground">
                              Add your first milestone to begin tracking the
                              project progress.
                            </p>
                          </div>
                        </div>
                              ...
                              <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-6">
                                <div className="flex gap-5">
                                  ...
                                  <div className="space-y-2 pt-1">
                                    <h5 className="font-bold text-xl leading-tight tracking-tight group-hover:text-primary transition-colors">
                                      {milestone.title}
                                    </h5>
                                ...
                                <div className="flex items-center gap-2">
                                  {isArtist &&
                                    (milestone.status === "ACTIVE" ||
                                      ...
                                      <Button
                                        onClick={() =>
                                          handleToggleMilestoneStatus(milestone)
                                        }
                                        size="sm"
                                        variant="outline"
                        ...
                        <div className="flex flex-col gap-6">
                          ...
                          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                            ...
                            <Button
                              ...
                              disabled={
                                addingMilestone || !newMilestone.title.trim()
                              }
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-sm text-muted-foreground">
                              Deliverables, references, and project files.
                            </p>
                          </div>
                        </div>
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                        {files.length === 0 ? (
                          ...
                          files.map((file) => (
                            <div
                              key={file.id}
                              className="group p-5 rounded-[2rem] border bg-white dark:bg-card/40 border-border/50 hover:border-primary/30 hover:shadow-xl transition-all duration-500 flex items-start gap-4"
                            >
                              ...
                              <div className="flex-1 min-w-0 pt-1">
                                ...
                                <div className="flex gap-2 mt-4">
                                  <Button
                                    variant="secondary"
                                    size="sm"
                                    className="h-10 rounded-xl px-4 font-bold text-[10px] uppercase tracking-wider hover:bg-primary hover:text-white transition-all"
                  ...
                  <TabsContent
                    id="project-tab-content-communication"
                    value="communication"
                    className="mt-0 outline-none focus-visible:ring-0"
                  >
                      ...
                      <div className="p-4 sm:p-5 border-b border-border/40 bg-muted/5 flex items-center gap-3 shrink-0">
                        ...
                        <div>
                          ...
                          <p className="text-[11px] text-muted-foreground">
                            Direct communication for this project.
                          </p>
                        </div>
                      </div>

                      <ScrollArea
                        className="flex-1 p-4 sm:p-5 h-full"
                        id="chat-scroll-area"
                      >
                          ...
                          {messages.length === 0 ? (
                            <div className="py-20 text-center space-y-4">
                              ...
                              <p className="text-muted-foreground font-medium max-w-[200px] mx-auto">
                                No messages yet. Send a quick update to get
                                started!
                              </p>
                            </div>
                          ) : (
                            messages.map((msg) => {
                              ...
                              return (
                                ...
                                >
                                  ...
                                  <div
                                    className={`max-w-[80%] space-y-1.5 ${isMe ? "items-end" : "items-start"}`}
                                  >
                                    <div
                                      className={`p-4 rounded-3xl text-sm leading-relaxed shadow-sm transition-all duration-300 hover:shadow-md ${isMe ? "bg-primary text-primary-foreground rounded-tr-none" : "bg-white dark:bg-card border border-border/50 rounded-tl-none"}`}
                                    >
                                      {msg.content}
                                    </div>
                                    <p
                                      className={`text-[10px] font-bold uppercase tracking-widest px-2 ${isMe ? "text-primary/60" : "text-muted-foreground/60"}`}
                                    >
                                      {formatDate(
                          ...
                          <div className="flex sm:flex-col justify-end gap-3">
                            ...
                            >
                              {sendingMessage ? (
                                ...
                                <SendHorizontal className="h-5 w-5 group-hover:translate-x-0.5 group-hover:-translate-y-0.5 transition-transform" />
                              )}
                            </Button>
                          </div>
                        </div>
