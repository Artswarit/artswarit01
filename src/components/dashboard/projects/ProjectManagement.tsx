...
const ProjectManagement = () => {
  ...
  const fetchProjects = useCallback(async () => {
    ...
    try {
      ...
      const transformedProjects = (data || []).map((project: any) => {
        ...
        const sourceCurrency = project.amount_usd
          ? "USD"
        ...
        return {
          ...
          client: project.client_id
            ? clientProfiles[project.client_id]?.full_name || "Client"
            : "Client",
          clientAvatar: project.client_id
            ? clientProfiles[project.client_id]?.avatar_url || undefined
            : undefined,
          progress:
            project.progress ??
          ...
          payment:
            displayAmount > 0
              ? format(displayAmount, sourceCurrency, project.exchange_rate)
              : "Not set",
  ...
  useEffect(() => {
    ...
    const milestonesChannel = supabase
      .channel(`milestones-realtime:${user.id}`)
      .on(
        "postgres_changes",
        {
          event: "*",
        ...
        async (payload) => {
          const projectId =
            ...
            (payload?.old as any)?.project_id;
          if (projectId) {
            // Verify project belongs to this artist before processing to prevent system-wide event processing
            const { data } = await supabase.from("projects").select("artist_id").eq("id", projectId).maybeSingle();
  ...
  const checkProjectReady = useCallback(async (projectId: string) => {
    ...
    const allPaid = milestones.every(
      (m) => Boolean(m.paid_at) || m.status === "COMPLETED",
    ); // Disputed can block completion
  ...
  const autoCompleteIfReady = useCallback(
    async (projectId: string, isReady: boolean) => {
      if (!isReady) return;
  ...
  const handleCompleteProject = async (
    ...
    try {
      ...
      const allPaidAndCompleted =
        (milestones || []).length > 0 &&
        (milestones || []).every(
          (m) => m.status === "COMPLETED" && !!m.paid_at,
        );
      const anyDispute = (milestones || []).some(
        (m) => m.status === "DISPUTED",
      );
      ...
      if (!allPaidAndCompleted || anyDispute) {
        toast.error(
          anyDispute
            ? "Cannot complete: a dispute is active."
            : "Complete all milestones and payments before marking the project complete.",
        );
        return;
      }
  ...
  const getProjectReview = (projectId: string) =>
    clientReviews[projectId] || null;
  const activeProjects = projects.filter((p) => p.status === "accepted");
  const pendingProjects = projects.filter(
    (p) => p.status === "pending" && p.is_locked,
  );
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-sm font-medium text-muted-foreground leading-relaxed">
            Manage client projects and track their progress
          </p>
        </div>
      </div>
      ...
      <Tabs defaultValue="active" className="w-full">
        <div className="overflow-x-auto no-scrollbar -mx-4 px-4 sm:mx-0 sm:px-0">
          <TabsList className="mb-6 w-full h-auto p-1.5 bg-muted/30 backdrop-blur-md rounded-2xl border border-border/10 flex items-stretch gap-1.5">
            <TabsTrigger
              value="active"
              className="flex-1 min-w-[100px] py-2.5 sm:py-3 rounded-xl data-[state=active]:bg-background data-[state=active]:shadow-lg data-[state=active]:text-primary transition-all font-black text-[9px] sm:text-[10px] uppercase tracking-wider sm:tracking-widest min-h-[44px] sm:min-h-[48px] px-2 sm:px-4 flex items-center justify-center"
            >
          ...
          {activeProjects.length > 0 ? (
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              {activeProjects.map((project) => (
                <Card
                  key={project.id}
                  className="rounded-[2rem] border-border/40 shadow-xl shadow-black/5 bg-background/50 backdrop-blur-md overflow-hidden group hover:border-primary/20 transition-all duration-300"
                >
                  <CardHeader className="pb-4">
                    <div className="flex justify-between items-start">
                      ...
                      <Badge className="bg-blue-500/10 text-blue-500 border-blue-500/20 rounded-full px-3 py-1 font-bold text-[10px] uppercase tracking-wider">
                        Active
                      </Badge>
                    </div>
                    <CardDescription className="pt-1">
                      {project.client_id ? (
                        <Link
                          ...
                          className="inline-flex items-center gap-2 font-bold text-xs text-primary hover:text-primary/80 transition-colors"
                        >
                  ...
                  <CardContent className="space-y-6">
                    ...
                    <div className="flex flex-wrap gap-4 pt-2">
                      {project.deadline && (
                        <div className="flex items-center gap-2 px-3 py-1.5 rounded-xl bg-muted/30 border border-border/10 text-xs font-bold text-muted-foreground">
                  ...
                  <CardFooter className="flex flex-wrap gap-3 p-5 sm:p-6 bg-muted/5 border-t border-border/10">
                    <Button
                      ...
                      className="flex-1 sm:flex-none h-12 rounded-xl font-black text-[10px] uppercase tracking-widest border-border/60 hover:bg-background hover:scale-[1.02] transition-all"
                    >
                    ...
                    <Select
                      ...
                      onValueChange={(value) =>
                        handleUpdateProgress(project, Number.parseInt(value))
                      }
                    ...
                    {readyMap[project.id] && project.status !== "completed" && (
                      <Button
                        size="sm"
                        className="flex-1 sm:flex-none bg-primary text-primary-foreground hover:bg-primary/90 h-12 rounded-xl font-black text-[10px] uppercase tracking-widest shadow-lg shadow-primary/20 hover:scale-[1.02] active:scale-[0.98] transition-all"
                        onClick={() => handleCompleteProject(project)}
                        disabled={
                          actionLoading === project.id ||
                          autoCompleting[project.id]
                        }
                      >
                        {actionLoading === project.id ||
                        autoCompleting[project.id] ? (
            ...
            <div className="text-center py-20 bg-muted/10 rounded-[2rem] border-2 border-dashed border-border/20">
                ...
                >
                  <CardHeader className="pb-4">
                    <div className="flex justify-between items-start">
                      ...
                      <Badge className="bg-amber-500/10 text-amber-500 border-amber-500/20 rounded-full px-3 py-1 font-bold text-[10px] uppercase tracking-wider">
                        Pending
                      </Badge>
                    </div>
                    <CardDescription className="pt-1">
                  ...
                  <CardFooter className="flex flex-wrap gap-3 p-5 sm:p-6 bg-muted/5 border-t border-border/10">
                    ...
                    <Button
                      ...
                      disabled={actionLoading === project.id}
                      className="flex-1 sm:flex-none h-12 rounded-xl font-black text-[10px] uppercase tracking-widest bg-primary text-primary-foreground shadow-lg shadow-primary/20 hover:scale-[1.02] active:scale-[0.98] transition-all"
                    >
                      {actionLoading === project.id ? (
                        <Loader2 className="h-4 w-4 animate-spin mr-2" />
                      ) : (
                    ...
                    <Button
                      size="sm"
                      variant="outline"
                      className="flex-1 sm:flex-none text-destructive border-destructive/40 hover:bg-destructive hover:text-destructive-foreground h-12 rounded-xl font-black text-[10px] uppercase tracking-widest hover:scale-[1.02] active:scale-[0.98] transition-all"
                  ...
                  >
                    <CardHeader className="pb-4">
                      <div className="flex justify-between items-start">
                        ...
                        <Badge className="bg-emerald-500/10 text-emerald-500 border-emerald-500/20 rounded-full px-3 py-1 font-bold text-[10px] uppercase tracking-wider">
                          Completed
                        </Badge>
                      </div>
                      <CardDescription className="pt-1">
                    ...
                    <CardContent className="space-y-6">
                      <div>
                        ...
                        <Progress
                          value={100}
                          className="h-2 rounded-full bg-emerald-500/10"
                        />
                      </div>
                          ...
                          <div className="flex flex-col items-end gap-1.5">
                            ...
                            <div className="flex items-center gap-0.5">
                              {[1, 2, 3, 4, 5].map((star) => (
                                <Star
                                  key={star}
                                  className={`w-3.5 h-3.5 ${star <= review.rating ? "fill-amber-400 text-amber-400" : "text-muted-foreground/20"}`}
                                />
                              ))}
                            </div>
                          </div>
                    ...
                    <CardFooter className="flex flex-wrap gap-3 p-5 sm:p-6 bg-muted/5 border-t border-border/10">
                      ...
                      <Button
                        ...
                        className={`flex-1 sm:flex-none h-12 rounded-xl font-black text-[10px] uppercase tracking-widest transition-all hover:scale-[1.02] active:scale-[0.98] ${review ? "border-border/60 hover:bg-background" : "bg-amber-500 text-white shadow-lg shadow-amber-500/20 hover:bg-amber-600"}`}
                      >
                        {review ? (
                          <>
