...
const ArtworkUploadForm = ({ onCancel, onSuccess }: ArtworkUploadFormProps) => {
  ...
  const handleFileSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files.length > 0) {
      ...
      const allowedTypes =
      ALLOWED_FORMATS[selectedType] || ALLOWED_FORMATS.image;

      // Validate file formats
      const invalidFormat = fileArray.find(
        (f) =>
        !allowedTypes.some(
          (t) => f.type.startsWith(t.split("/")[0]) || f.type === t
        )
      );
      if (invalidFormat) {
        toast({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const artworkData = {
      ...
      Number.parseFloat(
        (Number.parseFloat(price) / (exchangeRates[userCurrency] || 1)).toFixed(
          6
        )
      ) :
      null,
      ...
      <form onSubmit={handleSubmit} className="space-y-6 sm:space-y-10">
            ...
            <CardContent className="p-4 sm:p-6">
              <div className="flex flex-col sm:flex-row items-center gap-4 text-center sm:text-left">
                ...
                <div className="flex-1 space-y-1">
                  <p className="font-black text-orange-800 dark:text-orange-400 uppercase tracking-tight">
                    Portfolio limit reached
                  </p>
                  <p className="text-xs sm:text-sm text-orange-600/80 font-medium leading-relaxed">
                    You've uploaded {portfolioCount}/{portfolioLimit} items.
                    Upgrade to Pro for unlimited creative freedom!
                  </p>
                </div>
                <Button
                type="button"
        ...
        <Card className="rounded-[2rem] border-border/40 shadow-xl shadow-black/5 overflow-hidden">
          <CardHeader className="bg-muted/30 px-5 py-4 sm:px-8 sm:py-6">
            ...
            <CardDescription className="text-xs sm:text-sm font-medium opacity-70">
              ...
              {contentTypes.map((type) => {
                ...
                return (
                  <div
                    ...
                    onClick={() => setSelectedType(type.id)} role="button" tabIndex={0} onKeyDown={(e) => {if (e.key === "Enter" || e.key === " ") {e.preventDefault();(() => setSelectedType(type.id))(e);}}}>
        ...
        <Card className="rounded-[2rem] border-border/40 shadow-xl shadow-black/5">
          <CardHeader className="bg-muted/30 px-5 py-4 sm:px-8 sm:py-6">
            ...
            <CardDescription className="text-xs sm:text-sm font-medium opacity-70">
              Provide essential information about your artwork
            </CardDescription>
          </CardHeader>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 sm:gap-8">
              ...
              <div className="space-y-2.5 sm:space-y-3">
                ...
                <Select
                  ...
                  <SelectTrigger
                    id="category"
                    className="h-12 rounded-xl bg-muted/20 border-border/40 focus:ring-primary/20 focus:border-primary font-medium text-sm sm:text-base">
                  ...
                  <SelectContent className="rounded-2xl border-border/40 shadow-2xl">
                    ...
                    <SelectItem
                      key={cat}
                      value={cat}
                      className="font-medium rounded-lg m-1 text-sm sm:text-base">
              ...
              <div className="group relative border-2 border-dashed border-border/60 rounded-[2rem] p-5 sm:p-12 text-center cursor-pointer hover:border-primary/50 hover:bg-primary/5 transition-all duration-500 ease-out overflow-hidden">
                ...
                <Label
                  ...
                  <div className="mx-auto flex items-center justify-center w-14 h-14 sm:w-20 sm:h-20 rounded-[1.5rem] sm:rounded-[2rem] bg-primary/10 text-primary mb-4 sm:mb-6 group-hover:scale-110 transition-transform duration-500 shadow-inner">
                    ...
                    {selectedType === "audio" &&
                    <MusicIcon className="h-7 w-7 sm:h-10 sm:w-10" />
                    }
                    {selectedType === "video" &&
                    <VideoIcon className="h-7 w-7 sm:h-10 sm:w-10" />
                    }
                  </div>
                  ...
                  <div className="space-y-1.5 sm:space-y-2">
                      ...
                      <p className="text-[10px] sm:text-sm text-muted-foreground font-medium opacity-60">
                        High resolution files supported
                      </p>
                    </div> :
        ...
        <Card className="rounded-[2rem] border-border/40 shadow-xl shadow-black/5">
          <CardHeader className="bg-muted/30 px-5 py-4 sm:px-8 sm:py-6">
            ...
            <CardDescription className="text-xs sm:text-sm font-medium opacity-70">
              Define access rights and pricing strategy
            </CardDescription>
          </CardHeader>
          <CardContent className="p-5 sm:p-8 space-y-6 sm:space-y-8">
            ...
            <div className="space-y-3 sm:space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4">
                ...
                <div
                  ...
                  onClick={() => setVisibilityType(tier.id)} role="button" tabIndex={0} onKeyDown={(e) => {if (e.key === "Enter" || e.key === " ") {e.preventDefault();(() => setVisibilityType(tier.id))(e);}}}>
                    ...
                    <p className="text-[10px] sm:text-[11px] text-muted-foreground font-medium opacity-70 leading-tight">
                      {tier.desc}
                    </p>
                    {visibilityType === tier.id &&
            ...
            <div className="space-y-3 sm:space-y-4 p-5 sm:p-6 rounded-[2rem] bg-primary/5 border border-primary/10 animate-in zoom-in-95 duration-500">
                <Label
                htmlFor="price"
                className="text-[10px] font-black uppercase tracking-widest text-primary/80 ml-1">
                
                  Set Your Price ({userCurrency})*
                </Label>
                <div className="relative group">
                  <span className="absolute left-4 top-1/2 -translate-y-1/2 text-primary font-black text-base sm:text-lg">
                    {userCurrencySymbol}
                  </span>
                  <Input
                  id="price"
            ...
            <div className="flex items-center justify-between p-4 sm:p-6 rounded-2xl border border-border/40 bg-muted/10 group hover:border-primary/20 transition-all duration-300">
              <div className="space-y-0.5 sm:space-y-1">
                <Label
                  htmlFor="schedule"
                  className="font-black text-xs sm:text-sm uppercase tracking-tight group-hover:text-primary transition-colors cursor-pointer">
                  
                  Schedule Release
                </Label>
                <p className="text-[10px] sm:text-xs text-muted-foreground font-medium opacity-60">
                  Set a future date for this content to go live
                </p>
              </div>
              <Switch
                id="schedule"
            ...
            <div className="space-y-2.5 sm:space-y-3 animate-in fade-in slide-in-from-top-4 duration-500">
                ...
                <Popover>
                  <PopoverTrigger asChild>
                    <Button
                    variant="outline"
                    className="h-12 w-full justify-start text-left font-medium rounded-xl border-border/40 bg-muted/20 hover:bg-muted/30 transition-all text-sm sm:text-base">
                    
                      <CalendarIcon className="mr-3 h-4 w-4 text-primary" />
                      {releaseDate ?
                    format(releaseDate, "PPP") :

                    <span className="opacity-50">Pick a future date</span>
                    }
                    </Button>
                  </PopoverTrigger>
                  <PopoverContent
        ...
        <AlertDialogContent className="rounded-3xl border-border/40 shadow-2xl max-w-[90vw] sm:max-w-lg">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="text-sm font-medium leading-relaxed">
              Some files have been flagged as potential AI-generated content.
              While allowed, this may affect content visibility and discovery in
              certain regions.
              <br />
              <br />
