...
const ArtworkManagementCard = ({
  ...
  const accessType: "free" | "premium" | "exclusive" =
    (artwork.metadata?.access_type as "free" | "premium" | "exclusive") ||
    "free";
  const imageUrl = artwork.media_url;

  const [pendingStatus, setPendingStatus] = useState<
    "public" | "private" | "archived" | null
  >(null);
  ...
  const handleStatusChange = async (
    newStatus: "public" | "private" | "archived",
  ) => {
    if (artwork.metadata?.admin_banned) {
      toast({
        title: "Content Blocked",
        description:
          "This artwork was removed by a platform administrator and cannot be restored. Reason: " +
          (artwork.metadata?.ban_reason || "Community Guidelines Violation"),
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      if (newPinnedState && pinnedCount >= 5) {
        toast({
          ...
          description:
            "You can only pin up to 5 artworks. Unpin one to pin another.",
          variant: "destructive",
        });
        return;
      }
  ...
  const getStatusConfig = (status: string) => {
    if (artwork.metadata?.admin_banned) {
      return {
        ...
        icon: AlertTriangle,
        className:
          "bg-red-600/90 text-white border-red-500/50 shadow-sm shadow-red-500/20",
      };
    }
    ...
    switch (status) {
      case "public":
        return {
          ...
          icon: Globe,
          className:
            "bg-emerald-500/90 text-white border-emerald-400/50 shadow-sm shadow-emerald-500/20",
        };
      case "private":
        return {
          label: "Private",
          icon: Lock,
          className:
            "bg-amber-500/90 text-white border-amber-400/50 shadow-sm shadow-amber-500/20",
        };
      case "archived":
        return {
          label: "Archived",
          icon: Archive,
          className:
            "bg-slate-500/90 text-white border-slate-400/50 shadow-sm shadow-slate-500/20",
        };
      default:
        return {
          label: "Public",
    ...
    switch (type) {
      case "premium":
        return {
          label: "Premium",
          className:
            "bg-purple-600/90 text-white border-purple-400/50 shadow-sm shadow-purple-500/20",
        };
      case "exclusive":
        return {
          label: "Exclusive",
          className:
            "bg-fuchsia-600/90 text-white border-fuchsia-400/50 shadow-sm shadow-fuchsia-500/20",
        };
      default:
        return {
          label: "Free",
          className:
            "bg-sky-600/90 text-white border-sky-400/50 shadow-sm shadow-sky-500/20",
        };
    }
  };
        ...
        <div className="flex flex-col items-end gap-0.5 sm:gap-1 shrink-0 ml-auto">
          ...
          <div className="flex md:hidden items-center gap-1.5 sm:gap-2 text-[8px] sm:text-[10px] text-muted-foreground opacity-70">
            <span className="flex items-center gap-0.5">
              <Heart className="h-2.5 w-2.5" /> {formatNumber(likes)}
            </span>
            <span className="flex items-center gap-0.5">
              <Eye className="h-2.5 w-2.5" /> {formatNumber(views)}
            </span>
          </div>
        </div>
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button
                variant="ghost"
                size="icon"
                className="h-11 w-11 sm:h-12 sm:w-12 hover:bg-primary/10 hover:text-primary transition-colors"
              >
            ...
            <DropdownMenuContent align="end" className="w-48">
              ...
              <DropdownMenuItem asChild>
                <Link
                  ...
                  className="gap-2 cursor-pointer min-h-[44px] sm:min-h-[40px]"
                >
              ...
              <DropdownMenuItem
                onClick={onDelete}
                className="gap-2 text-destructive focus:text-destructive min-h-[44px] sm:min-h-[40px]"
              >
    ...
    >
      ...
      <div className="relative aspect-[4/3] overflow-hidden rounded-t-xl">
        ...
        <div className="absolute top-3 right-3 flex flex-col gap-1.5 items-end">
          <Badge
            className={cn(
              "gap-1 px-2 py-1 backdrop-blur-md border border-white/20",
              statusConfig.className,
            )}
          >
          ...
          <Badge
            className={cn(
              "gap-1 px-2 py-1 backdrop-blur-md border border-white/20 text-[9px]",
              accessConfig.className,
            )}
          >
            {accessConfig.label}
        ...
        {isPinned && (
          <div className="absolute top-3 left-10">
            <Badge
              variant="secondary"
              className="gap-1 px-2 py-1 bg-primary text-primary-foreground border-none"
            >
