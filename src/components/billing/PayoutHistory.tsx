...
export function PayoutHistory() {
  ...
  const fetchPayouts = useCallback(async () => {
    ...
    try {
      ...
      const clientIds = [
        ...new Set(
          payments?.map((p) => p.milestone?.project?.client_id).filter(Boolean),
        ),
      ];
      ...
      const transformedPayouts: Payout[] = (payments || []).map((payment) => ({
        ...
        clientName:
          clientProfiles[payment.milestone?.project?.client_id] ||
          "Unknown Client",
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              ...
              <CardDescription>
                Your completed and pending payouts from milestones
              </CardDescription>
            </div>
                    ...
                    <div className="flex flex-col items-end gap-2">
                      <div className="text-right">
                        <p className="font-semibold text-lg">
                          {format(payout.artistPayout)}
                        </p>
                        <p className="text-xs text-muted-foreground">
                          Fee: {format(payout.platformFee)} (
                          {Math.round(
                            (payout.platformFee / payout.amount) * 100,
                          )}
                          %)
                        </p>
                      </div>
