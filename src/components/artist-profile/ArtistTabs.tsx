...
const ServicesTab = ({
  ...
  const submitRequest = () => {
    toast({ title: "Project request sent!", description: "The artist will be notified of your interest." });
    reset();
  };
  return (
    <div className="my-6 max-w-2xl mx-auto">
      <h3 className="font-bold text-xl text-purple-900 mb-3 flex items-center gap-2">
        <Mail className="text-purple-500" size={22} /> Services & Project Request
      </h3>
      {services.length === 0 ? (
        ...
        <div className="grid gap-4 mb-7">
          {services.map((service) => (
            <div key={service.id} className="p-4 rounded-xl border bg-white/60 shadow flex flex-col md:flex-row justify-between items-start md:items-center">
              <div>
      ...
      <form onSubmit={handleSubmit(submitRequest)} className="bg-white/80 rounded-xl p-6 shadow space-y-4">
        ...
        <Button type="submit" disabled={isSubmitting} className="bg-violet-600 text-white hover:bg-violet-700 font-semibold gap-2 flex items-center">
          <Mail size={17} /> {isSubmitting ? "Sending..." : "Send Request"}
        </Button>
      </form>
    </div>
  );
...
const AboutTab = ({
  ...
  return (
    <div className="my-8 px-2 max-w-xl mx-auto">
      <h3 className="font-bold text-xl text-purple-900 mb-2">{aboutDetails.artist.name}</h3>
      <div className="flex flex-col gap-2 mb-4 text-[15px]">
        ...
        {aboutDetails.artist.email && (
          <div className="flex items-center gap-2 text-gray-600">
            ...
            <a href={`mailto:${aboutDetails.artist.email}`} className="hover:underline">{aboutDetails.artist.email}</a>
          </div>
        )}
        ...
        {reviewCount === 0 ? (
          ...
          <div className="space-y-4">
            {reviews.map((rev) => {
              ...
              return (
                <div key={rev.id} id={`review-${rev.id}`} className={isHighlighted ? "scroll-mt-24 rounded-lg ring-2 ring-primary/30" : "scroll-mt-24"}>
                  <ReviewCard
                    reviewId={rev.id}
                    artistId={artistId}
...
const ArtGrid = ({
  ...
  handleRequestAccess: (id: string, name?: string) => void;
}) => (
  <>
    ...
    {paged.length === 0 && (
      <div className="text-muted-foreground text-center py-10">No art in this section.</div>
    )}
  </>
);
...
const ArtistTabs: React.FC<ArtistTabsProps> = ({
  artistId, allArt, premiumArt, exclusiveArt, pinnedIds = [],
  aboutDetails, services = [], reviews = [], onArtworkClick,
  isArtistOwner = false, onRefreshReviews, currentUserId,
}) => {
  ...
  useEffect(() => {
    async function fetchUnlockedArtworks() {
      if (!user?.id) return;
      const { data } = await supabase.from("artwork_unlocks").select("artwork_id").eq("user_id", user.id);
      if (data) setUnlockedArtworkIds(new Set(data.map((u) => u.artwork_id)));
    }
  ...
  useEffect(() => {
    ...
    async function fetchExclusiveStatus() {
      ...
      const { data, error } = await supabase.from("exclusive_memberships").select("status").eq("artist_id", artistId).eq("client_id", user.id).maybeSingle();
      if (!error && data) setExclusiveStatus(normalizeExclusiveStatus((data as any).status));
      else if (!error && !data) setExclusiveStatus("none");
      setExclusiveLoading(false);
    }
    fetchExclusiveStatus();
    if (user?.id && artistId) {
      channel = supabase.channel(`exclusive-membership-${artistId}-${user.id}`)
        .on("postgres_changes", { event: "*", schema: "public", table: "exclusive_memberships", filter: `client_id=eq.${user.id}` },
          (payload) => {
  ...
  const handleRequestAccess = useCallback(async (artworkId: string, artistName?: string) => {
    if (!user) {
      toast({ title: "Login Required", description: "Please log in to request access.", variant: "destructive" });
      return;
    }
    if (exclusiveStatus === "approved") { toast({ title: "Already Approved", description: "You already have access." }); return; }
    if (exclusiveStatus === "pending") { toast({ title: "Request Pending", description: "Your request is pending." }); return; }
    setExclusiveLoading(true);
    try {
      const { data: existingProfile } = await supabase.from("profiles").select("id").eq("id", user.id).maybeSingle();
      if (!existingProfile) {
        await supabase.from("profiles").insert({ id: user.id, email: user.email || "", full_name: (user as any).user_metadata?.full_name || user.email?.split("@")[0] || "User", role: "client" });
      }
      const { error } = await supabase.from("exclusive_memberships").upsert({ artist_id: artistId, client_id: user.id, status: "pending" }, { onConflict: "artist_id,client_id" });
      if (error) { toast({ title: "Request Failed", description: error.message, variant: "destructive" }); return; }
      const clientName = (user as any)?.user_metadata?.full_name || user.email?.split("@")[0] || "A client";
      if (artistId && artistId !== user.id) {
        await supabase.from("notifications").insert({ user_id: artistId, title: "New Exclusive Access Request", message: `${clientName} requested exclusive access.`, type: "info", metadata: { artist_id: artistId, client_id: user.id } });
      }
      setExclusiveStatus("pending");
      toast({ title: "Access Requested", description: `Your request has been sent to ${artistName || "the artist"}.` });
    } catch (err: any) {
  ...
  useEffect(() => {
    ...
    if (["all", "premium", "exclusive", "services", "about"].includes(normalized)) {
      setTab(normalized); setPage(1);
    }
  }, [searchParams]);
  ...
  const displayed: Record<string, GalleryArtwork[]> = { all: allWithPinnedFirst, premium: premiumArt, exclusive: exclusiveArt };
    ...
    <div>
      <Tabs value={tab} onValueChange={(v) => { setTab(v); setPage(1); }}>
        ...
        <TabsContent value={tab}>
          ...
          {tab === "services" && (
            <ServicesTab services={services} formatPlus={formatPlus} userCurrencySymbol={userCurrencySymbol} />
          )}
          {tab === "about" && aboutDetails && (
            <AboutTab
              aboutDetails={aboutDetails} reviews={reviews} artistId={artistId}
              isArtistOwner={isArtistOwner} currentUserId={currentUserId}
              onRefreshReviews={onRefreshReviews} searchParams={searchParams}
            />
          )}
        </TabsContent>
      </Tabs>
