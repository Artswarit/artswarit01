...
const TrendingAlgorithm = () => {
  ...
  const fetchTrendingData = async () => {
    try {
      ...
      const getCounts = (data: any[] | null) => {
        ...
        data?.forEach((item) =>
          map.set(item.artwork_id, (map.get(item.artwork_id) || 0) + 1),
        );
        return map;
      };
      ...
      const trending: TrendingItem[] = artworks.map((artwork) => {
        ...
        const interactionScore =
          recentS * 15 + recentC * 10 + recentL * 5 + recentV * 1;
        ...
        const ageHours = Math.max(
          1,
          (now.getTime() - new Date(artwork.created_at).getTime()) /
            (1000 * 60 * 60),
        );
        ...
        const relationshipBoost = followingIds.has(artwork.artist_id)
          ? 1.4
          : 1.0;

        // Final Composite Score
        const trendingScore = Math.floor(
          (interactionScore + velocity * 10) *
            decay *
            freshnessBoost *
            relationshipBoost *
            1000,
        ...
        const trendingChange = Math.floor(
          (interactionScore / (metadata?.likes_count || 1)) * 100,
        );
  ...
  const categories = useMemo(() => {
    ...
    const foundCategories = [
      ...new Set(trendingItems.map((item) => item.category)),
    ].filter(Boolean);
          ...
          <div className="relative z-10 flex flex-col md:flex-row md:items-end justify-between gap-6">
            <div className="space-y-2">
              ...
              <p className="text-muted-foreground text-sm sm:text-lg max-w-2xl font-medium">
                Discover the most engaging artworks currently rising in the
                community.
              </p>
            </div>
              ...
              <TooltipContent className="p-4 max-w-xs bg-slate-900 text-white border-none rounded-2xl shadow-2xl">
                <div className="space-y-2">
                  ...
                  <p className="text-xs leading-relaxed text-slate-300">
                    Our algorithm calculates "Engagement Velocity" using
                    gravity-based decay. Recent likes, views, and shares are
                    weighted higher than older activity to ensure the feed stays
                    fresh.
                  </p>
                </div>
              </TooltipContent>
              ...
              >
                <div className="bg-slate-50 rounded-3xl p-10 inline-block border border-dashed border-slate-200">
                  ...
                  <p className="text-slate-500 font-bold">
                    No trending items found in this category.
                  </p>
                </div>
              </motion.div>
            ) : (
