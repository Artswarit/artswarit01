  ...
  return (
    <div className="w-full space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-4xl mx-auto">
        {/* Starter Plan */}
        <Card
          className={`relative transition-all ${currentPlan === "starter" ? "ring-2 ring-primary" : "hover:shadow-lg"}`}
        >
          {currentPlan === "starter" && (
            ...
            <ul className="space-y-3 mb-6">
              {starterFeatures.map((feature, index) => (
                <li key={index} className="flex items-center gap-3">
                  <div
                    className={`flex-shrink-0 ${feature.isNegative ? "text-orange-500" : "text-green-500"}`}
                  >
                    {feature.included ? (
                  ...
                  <span
                    className={`text-sm ${feature.isNegative ? "text-orange-600 font-medium" : ""}`}
                  >
                    {feature.text}
                  </span>
                </li>
        ...
        {/* Pro Plan */}
        <Card
          className={`relative transition-all border-2 ${currentPlan === "pro" ? "ring-2 ring-yellow-400" : "border-yellow-400 hover:shadow-xl"} bg-gradient-to-br from-yellow-50 via-orange-50 to-amber-50`}
        >
          {currentPlan === "pro" ? (
            <div className="absolute -top-3 left-4">
          ...
          <CardContent className="pt-0">
            <ul className="space-y-3 mb-6">
              {proFeatures.map((feature, index) => (
                <li key={index} className="flex items-center gap-3">
                  <div
                    className={`flex-shrink-0 ${feature.isHighlight ? "text-yellow-500" : "text-green-500"}`}
                  >
                    <CheckCircle className="h-5 w-5" />
                  </div>
                  <span
                    className={`text-sm ${feature.isHighlight ? "text-yellow-700 font-bold" : ""}`}
                  >
                    {feature.text}
                  </span>
                </li>
