    ...
    <div className="w-full space-y-8">
      ...
      <Card className="relative overflow-hidden border-0 bg-gradient-to-br from-yellow-50 via-orange-50 to-amber-50">
        ...
        <CardHeader className="relative z-10 text-center pb-4">
          ...
          <p className="text-gray-600 text-lg max-w-2xl mx-auto">
            Get exclusive features, boost your visibility, and unlock premium
            client projects with our subscription plans.
          </p>
        </CardHeader>
        ...
        <CardContent className="relative z-10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4 mb-8">
            {premiumFeatures.map((feature, index) => (
              <div
                key={index}
                className="bg-white/80 backdrop-blur-sm border border-white/60 rounded-xl p-4 text-center hover:bg-white/90 transition-all duration-200"
              >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-5xl mx-auto">
        {plans.map((plan) => (
          <Card
            key={plan.key}
            className={`relative overflow-hidden transition-all duration-300 hover:shadow-xl ${plan.popular ? "border-yellow-400 ring-2 ring-yellow-400 shadow-lg scale-105" : "border-gray-200 hover:border-yellow-300"}`}
          >
            {plan.popular && (
            ...
            <CardHeader className="text-center pb-4">
              <CardTitle className="text-xl font-bold text-gray-800 py-[12px]">
                {plan.name}
              </CardTitle>
              <div className="space-y-2">
                ...
                <div className="text-xs text-gray-600 h-8">
                  {plan.description}
                </div>
                {plan.savings && (
