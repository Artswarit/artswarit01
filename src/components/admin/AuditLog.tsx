...
> = {
  DISPUTE_NEUTRAL_HOLD: {
    ...
    label: "Dispute — Neutral Hold",
    category: "dispute",
  },
  DISPUTE_RELEASE_ARTIST: {
    icon: Wallet,
    color: "text-green-600 bg-green-500/10",
    label: "Dispute — Released to Artist",
    category: "dispute",
  },
  DISPUTE_REFUND_CLIENT: {
    icon: CreditCard,
    color: "text-blue-600 bg-blue-500/10",
    label: "Dispute — Refunded to Client",
    category: "dispute",
  },
  CONTENT_REMOVED: {
    icon: Trash2,
  ...
  USER_WARNING: {
    ...
    label: "User Warning Issued",
    category: "user",
  },
  USER_SHADOWBAN: {
    icon: EyeOff,
    color: "text-slate-600 bg-slate-500/10",
    label: "User Shadowbanned",
    category: "user",
  },
  USER_SUSPEND: {
    icon: Clock,
  ...
  USER_PERMANENT_BAN: {
    icon: Ban,
    color: "text-red-600 bg-red-500/10",
    label: "User Permanently Banned",
    category: "user",
  },
  USER_UNBAN_PROBATION: {
    icon: RotateCcw,
    color: "text-amber-600 bg-amber-500/10",
    label: "User Unbanned → Probation",
    category: "user",
  },
  USER_RESTRICTION_LIFTED: {
    icon: UserCheck,
    color: "text-green-600 bg-green-500/10",
    label: "User Restriction Lifted",
    category: "user",
  },
};
...
export default function AuditLog() {
  ...
  const filtered = entries.filter((e) => {
    ...
    const matchesCategory =
      filterCategory === "all" ||
      getConfig(e.action).category === filterCategory;
        ...
        <div className="p-3 rounded-xl bg-amber-50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-800/30 mb-6 flex items-start gap-2">
          ...
          <div>
            <p className="text-xs font-bold text-amber-700">
              Strict Enforcement Log
            </p>
            <p className="text-[10px] text-amber-600 mt-0.5">
              Entries recorded in this register are permanent and cannot be
              deleted or modified.
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-4 gap-2 mb-5">
          ...
          <div className="p-2 rounded-lg bg-muted/50 border text-center">
            <p className="text-lg font-black text-amber-600">
              {
                entries.filter(
                  (e) => getConfig(e.action).category === "dispute",
                ).length
              }
            </p>
            <p className="text-[9px] font-bold uppercase tracking-widest text-muted-foreground">
          ...
          <div className="p-2 rounded-lg bg-muted/50 border text-center">
            <p className="text-lg font-black text-red-600">
              {
                entries.filter(
                  (e) => getConfig(e.action).category === "moderation",
                ).length
              }
            </p>
            <p className="text-[9px] font-bold uppercase tracking-widest text-muted-foreground">
          ...
          <div className="p-2 rounded-lg bg-muted/50 border text-center">
            <p className="text-lg font-black text-indigo-600">
              {
                entries.filter((e) => getConfig(e.action).category === "user")
                  .length
              }
            </p>
            <p className="text-[9px] font-bold uppercase tracking-widest text-muted-foreground">
        ...
        {filtered.length === 0 ? (
          <div className="text-center py-12 text-muted-foreground">
            ...
            <p className="text-xs mt-1">
              Audit entries will appear here as admin actions are taken in the
              other panels.
            </p>
          </div>
        ) : (
            ...
            {visible.map((entry, i) => {
              ...
              return (
                ...
                >
                  {/* Icon */}
                  <div
                    className={`p-1.5 rounded-lg ${config.color} shrink-0 mt-0.5`}
                  >
                          ...
                          {Object.entries(entry.metadata)
                            ...
                            .map(([k, v]) => (
                              <Badge
                                key={k}
                                variant="outline"
                                className="text-[8px] font-mono bg-muted/50"
                              >
                                {k}:{" "}
                                {typeof v === "object"
                                  ? JSON.stringify(v)
