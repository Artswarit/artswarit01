...
export default function UserGovernance() {
  ...
  const fetchUsers = useCallback(async () => {
    setLoading(true);
    try {
      const [profilesRes, adminRolesRes, warningsRes, payoutRes] =
        await Promise.all([
          supabase
            .from("profiles")
            .select(
              "id, full_name, email, role, account_status, avatar_url, created_at",
            )
            .order("created_at", { ascending: false }),
          supabase.from("user_roles").select("user_id").eq("role", "admin"),
          supabase
            .from("user_warnings")
            .select("user_id, type, reason, expires_at, is_active")
            .eq("is_active", true),
          supabase
            .from("razorpay_accounts")
            .select("user_id, kyc_status, payouts_enabled"),
        ]);
      ...
      const warningMap: Record<
        string,
        { type: string; reason: string; expires_at: string | null }
      > = {};
      ...
      (warningsRes.data || []).forEach((w: any) => {
        if (
          !warningMap[w.user_id] ||
          (severity[w.type] || 0) > (severity[warningMap[w.user_id].type] || 0)
        ) {
          warningMap[w.user_id] = {
            type: w.type,
            reason: w.reason,
    ...
    try {
      for (const tid of targetIds) {
        switch (actionType) {
          case "notify":
            await supabase
              ...
              .insert({
                user_id: tid,
                title: "System Notice",
                message: customNotifyText || reason,
                type: "info",
              });
            await writeAuditLog(
              user?.id || "system",
          ...
          case "suspend":
            const exp = new Date(
              Date.now() + Number.parseInt(suspendDays) * 86400000,
            ).toISOString();
            await supabase
              .from("user_warnings")
      ...
      toast.success(
        selectedUser
          ? "Action complete"
          : `Action applied to ${targetIds.length} users`,
      );
      fetchUsers();
      setDialogOpen(false);
      setSelectedIds(new Set());
  ...
  const getStatusBadge = (s: string) => {
    ...
    if (s === "suspended")
      return (
        <Badge
          variant="secondary"
          className="bg-orange-500/10 text-orange-600 hover:bg-orange-500/20 font-semibold text-[10px] uppercase tracking-wide"
        >
          Suspended
        </Badge>
      );
    if (s === "warned")
      return (
        <Badge
          variant="secondary"
          className="bg-amber-500/10 text-amber-600 hover:bg-amber-500/20 font-semibold text-[10px] uppercase tracking-wide"
        >
          Warned
        </Badge>
      );
    return (
      <Badge
        variant="secondary"
        className="bg-green-500/10 text-green-600 hover:bg-green-500/20 font-semibold text-[10px] uppercase tracking-wide"
      >
        Active
      </Badge>
    );
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 pb-6 border-b">
            <div className="space-y-1">
              ...
              <p className="text-sm text-muted-foreground">
                Monitor accounts, enforce compliance, and maintain platform
                security.
              </p>
            </div>
                    ...
                    >
                      ...
                      <TableCell>
                        {u.is_admin ? (
                          <Badge
                            variant="default"
                            className="font-bold text-[10px] tracking-wide uppercase shadow-sm"
                          >
                            Admin
                          </Badge>
                        ) : (
                          <Badge
                            variant="outline"
                            className="font-medium text-[10px] tracking-wide uppercase"
                          >
                            {u.role}
                          </Badge>
                        )}
                      ...
                      <TableCell>{getStatusBadge(getStatus(u))}</TableCell>
                      <TableCell className="text-right pr-4">
                        <Button
                          variant="secondary"
                          size="sm"
                          onClick={() => openAction(u)}
                          className="h-8 shadow-sm font-semibold transition-all"
                        >
                          Manage
                        </Button>
                      </TableCell>
          ...
          <div className="bg-foreground text-background shadow-2xl rounded-2xl p-2 flex items-center justify-between border border-border">
            ...
            <div className="flex items-center gap-1">
              ...
              <Button
                ...
                className="h-9 px-4 rounded-xl bg-background text-foreground hover:bg-muted font-bold tracking-tight border shadow-sm"
              >
              ...
              <Button
                ...
                className="h-9 w-9 p-0 rounded-xl text-muted hover:text-background hover:bg-background/20"
                title="Deselect All"
              >
                <X className="h-4 w-4" />
              </Button>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            ...
            <DialogDescription>
              {selectedUser
                ? `Acting upon user: ${selectedUser.full_name || selectedUser.email}`
                : `Bulk intervention on ${selectedIds.size} users.`}
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-4 py-4">
                ...
                <SelectContent>
                  <SelectItem value="notify">
                    <div className="flex items-center gap-2 font-medium">
                      <Mail className="h-4 w-4 text-primary" /> Direct System
                      Message
                    </div>
                  </SelectItem>
                  ...
                  <SelectItem value="suspend">
                    <div className="flex items-center gap-2 font-medium">
                      <Clock className="h-4 w-4 text-orange-500" /> Temporary
                      Suspension
                    </div>
                  </SelectItem>
                  <SelectItem value="ban">
                    <div className="flex items-center gap-2 font-medium">
                      <Ban className="h-4 w-4 text-destructive" /> Permanent Ban
                    </div>
                  </SelectItem>
                  <SelectItem value="lift">
                    <div className="flex items-center gap-2 font-medium">
                      <RotateCcw className="h-4 w-4 text-green-500" /> Restore
                      Account
                    </div>
                  </SelectItem>
                </SelectContent>
            ...
            {actionType === "notify" && (
              <div className="space-y-2 animate-in slide-in-from-top-2">
                ...
                <Textarea
                  placeholder="Type the exact message users will see..."
            ...
            <div className="space-y-2">
              ...
              <Textarea
                placeholder="Reasoning for audit compliance... (Mandatory)"
                value={reason}
          ...
          <DialogFooter>
            ...
            <Button
              ...
              disabled={processing || !actionType || !reason.trim()}
