  ...
  return (
    <nav className="bg-white/80 dark:bg-card/80 backdrop-blur-xl border-b border-muted/20 fixed w-full top-0 z-50 transition-all duration-300 pt-[var(--safe-top)]">
          ...
          {user ? (
            <div className="flex items-center gap-1 sm:gap-3">
              ...
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <Button
                    variant="ghost"
                    className="relative h-10 w-10 sm:h-12 sm:w-12 p-0 rounded-2xl hover:bg-primary/5 transition-all duration-300 group"
                  >
                ...
                <DropdownMenuContent
                  className="w-56 p-2 z-50 rounded-2xl border-muted/20 bg-white/95 dark:bg-card/95 backdrop-blur-xl shadow-2xl"
                  align="end"
                  sideOffset={8}
                >
                  ...
                  <DropdownMenuItem
                    asChild
                    className="rounded-xl focus:bg-primary/5 focus:text-primary cursor-pointer py-2.5 font-bold"
                  >
                    <Link
                      to={
                        isAdmin
                  ...
                  <DropdownMenuItem
                    onClick={signOut}
                    className="rounded-xl focus:bg-red-50 focus:text-red-600 dark:focus:bg-red-500/10 cursor-pointer py-2.5 font-bold text-red-500"
                  >
                    Logout
                  </DropdownMenuItem>
                </DropdownMenuContent>
            ...
            <div className="hidden sm:flex items-center gap-3">
              <Button
                variant="ghost"
                asChild
                className="font-bold text-sm px-6 rounded-xl hover:bg-primary/5 hover:text-primary transition-all"
              >
                <Link to="/login">Login</Link>
              </Button>
              <Button
                asChild
                className="bg-primary hover:bg-primary/90 text-primary-foreground font-black text-sm px-6 rounded-xl shadow-lg shadow-primary/20 transition-all hover:scale-105 active:scale-95"
              >
      ...
      {isOpen && (
        <div
          id="mobile-menu"
          className="lg:hidden absolute top-full left-0 right-0 p-4 animate-in fade-in slide-in-from-top-4 duration-300 z-50"
        >
              ...
              {!user && (
                <div className="grid grid-cols-2 gap-3 pt-4 border-t border-muted/20 mt-4">
                  <Button
                    variant="outline"
                    asChild
                    className="rounded-2xl h-14 font-bold border-muted/30 hover:bg-primary/5 hover:text-primary transition-all"
                    onClick={closeMenu}
                  >
                  ...
                  <Button
                    asChild
                    className="rounded-2xl h-14 font-black bg-primary hover:bg-primary/90 text-primary-foreground shadow-lg shadow-primary/20 transition-all"
                    onClick={closeMenu}
                  >
