...
import {
  useArtworkFeedback,
  Feedback as FeedbackType } from
"@/hooks/useArtworkFeedback";
...
}: {name: string;avatarUrl: string | null;userId: string;}) =>
...
const UsernameLink = ({
...
}: {name: string;avatarUrl: string | null;userId: string;className?: string;}) => {
  ...
  return (
    <span
      ...
      {showCard &&
      <MiniProfileCard name={name} avatarUrl={avatarUrl} userId={userId} />
      }
      <Link
        to={`/artist/${userId}`}
        className={cn(
...
content: string,
userMap: Map<string, {id: string;avatarUrl: string | null;}>) =>
{
  ...
  const escapedUsernames = sortedUsernames.map(
    (u) => "@" + u.replaceAll(/[.*+?^${}()|[\]\\]/g, "\\$&")
  );
...
const CommentItem = ({
...
}: {feedback: FeedbackType & {user_id?: string;reply_count?: number;};artworkId: string;onReplyToParent?: (username: string, parentId: string) => void;}) => {
  const { user } = useAuth();
  ...
  const userMap = useMemo(() => {
    ...
    if (feedback.profiles?.full_name && feedback.user_id) {
      map.set(feedback.profiles.full_name, {
        id: feedback.user_id,
        avatarUrl: feedback.profiles.avatar_url ?? null
      });
    }
    replies.forEach((reply) => {
      if (reply.profiles?.full_name && reply.user_id) {
        map.set(reply.profiles.full_name, {
          id: reply.user_id,
  ...
  const timeAgo = formatDistanceToNow(new Date(feedback.created_at), {
    addSuffix: false
  });
        ...
        <div className="flex-1 min-w-0">
          <div className="text-sm leading-snug">
            ...
            <span className="text-foreground/90">
              {renderContentWithMentions(feedback.content, userMap)}
            </span>
          </div>
          {feedback.rating &&
          <StarRating
                    ...
                    className="shrink-0 cursor-pointer">
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        className="mr-1 text-[13px]" />
                          ...
                          <span className="text-foreground/90">
                            {renderContentWithMentions(reply.content, userMap)}
                          </span>
                        </div>
...
const ArtworkFeedback = ({
  ...
  const { user } = useAuth();
  const { toast } = useToast();
  const { feedback, isLoading, error, addFeedback, isAddingFeedback } =
  useArtworkFeedback(artworkId);
  ...
  const handleSubmit = () => {
    ...
    addFeedback(
      { content: newComment, rating: newRating > 0 ? newRating : null },
      {
        onSuccess: () => {
          setNewComment("");
          setNewRating(0);
