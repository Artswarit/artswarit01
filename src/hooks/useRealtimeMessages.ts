      ...
      try {
        ...
        const conversationsWithDetails = await Promise.all(
          (convData || []).map(async (conv) => {
            const otherUserId =
              conv.client_id === user.id ? conv.artist_id : conv.client_id;
            ...
            // Get last message
            const clearedAtForLastMsg =
              conv.client_id === user.id
                ? conv.client_last_cleared_at
                : conv.artist_last_cleared_at;
            ...
            return {
              ...
              lastMessageTime: lastMsgData?.created_at
                ? new Date(lastMsgData.created_at)
                : null,
        ...
        (payload) => {
          if (payload.eventType === "INSERT") {
            ...
            if (newMsg.conversation_id === activeConversationId) {
              ...
              if (
                clearedAt &&
                new Date(newMsg.created_at) <= new Date(clearedAt)
              ) {
                return;
              }
            ...
            if (
              updatedMsg.conversation_id === activeConversationId &&
