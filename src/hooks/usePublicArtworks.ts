  ...
  const fetchPage = useCallback(
    async (pageIndex: number, append: boolean = false) => {
      try {
        ...
        const artistIds = [
          ...new Set((artworksData || []).map((a) => a.artist_id)),
        ];
        ...
        const artistMap = new Map(
          (artistsData || []).map((a) => [
            a.id,
            { name: a.full_name, location: a.location },
          ]),
        );
        ...
        (likesResult.data || []).forEach((r) => {
          liveLikes.set(r.artwork_id, (liveLikes.get(r.artwork_id) || 0) + 1);
        });
        (viewsResult.data || []).forEach((r) => {
          liveViews.set(r.artwork_id, (liveViews.get(r.artwork_id) || 0) + 1);
        });

        // Transform data to match component expectations
        const transformedArtworks: PublicArtwork[] = freeArtworks.map(
          (artwork) => {
            ...
            return {
              ...
              audioUrl:
                artwork.media_type === "audio" ? artwork.media_url : null,
              videoUrl:
                artwork.media_type === "video" ? artwork.media_url : null,
            };
          },
        );

        setArtworks((prev) =>
          append ? [...prev, ...transformedArtworks] : transformedArtworks,
        );
        setPage(pageIndex);
