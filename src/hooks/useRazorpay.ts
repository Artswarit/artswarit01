...
export function useRazorpay() {
  ...
  const initiatePayment = useCallback(
    async ({ milestoneId, onSuccess, onFailure }: PaymentOptions) => {
      setLoading(true);
      ...
      try {
        ...
        if (!loaded) {
          throw new Error(
            "Failed to load payment gateway. Please check your internet connection or disable ad-blockers.",
          );
        }
        ...
        const orderResp = await fetch(
          `${SUPABASE_URL}/functions/v1/create-milestone-order`,
          {
            method: "POST",
            headers: {
        ...
        if (!orderResp.ok || orderData?.success === false) {
          ...
          throw new Error(
            orderData?.error ||
              `Failed to create payment order (HTTP ${orderResp.status})`,
          );
        }
