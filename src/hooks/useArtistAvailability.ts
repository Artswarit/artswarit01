...
export function useArtistAvailability(artistId?: string) {
  ...
  const [nextVacationEnd, setNextVacationEnd] = useState<string | undefined>(
    undefined,
  );
  const [nextVacationNote, setNextVacationNote] = useState<string | undefined>(
    undefined,
  );
  const [nextVacationStart, setNextVacationStart] = useState<
    string | undefined
  >(undefined);
  const [nextBusyStart, setNextBusyStart] = useState<string | undefined>(
    undefined,
  );
  const [nextBusyEnd, setNextBusyEnd] = useState<string | undefined>(undefined);
  const [nextBusyNote, setNextBusyNote] = useState<string | undefined>(
    undefined,
  );
  const [lastVacationSetDate, setLastVacationSetDate] = useState<
    string | undefined
  >(undefined);
  const [lastBusySetDate, setLastBusySetDate] = useState<string | undefined>(
    undefined,
  );
    ...
    async (startDate?: Date, endDate?: Date) => {
      ...
      try {
        ...
        if (!upcomingVac || upcomingVac.length === 0) {
          ...
          const monthVac = (data || []).filter(
            (d) => d.status === "vacation" && d.date >= today,
          );
        ...
        // Last set date (<= today)
        const vacPast = (data || []).filter(
          (d) => d.status === "vacation" && d.date <= todayStr,
        );
        if (vacPast.length > 0) {
          const latest = vacPast.sort((a, b) => (a.date > b.date ? 1 : -1))[
            vacPast.length - 1
          ];
        ...
        if (!upcomingBusy || upcomingBusy.length === 0) {
          const today = format(new Date(), "yyyy-MM-dd");
          const monthBusy = (data || []).filter(
            (d) => d.status === "busy" && d.date >= today,
          );
        ...
        const busyPast = (data || []).filter(
          (d) => d.status === "busy" && d.date <= todayStr,
        );
        if (busyPast.length > 0) {
          const latestBusy = busyPast.sort((a, b) =>
            a.date > b.date ? 1 : -1,
          )[busyPast.length - 1];
  ...
  const setDateAvailability = useCallback(
    async (date: Date, status: AvailabilityStatus, note?: string) => {
  ...
  const setMultiDateAvailability = useCallback(
    async (dates: Date[], status: AvailabilityStatus, note?: string) => {
      if (!user?.id || user.id !== targetArtistId) {
        toast({
          title: "Permission denied",
          description: "You can only update your own availability.",
  ...
  const clearDateAvailability = useCallback(
    async (date: Date) => {
      if (!user?.id || user.id !== targetArtistId) {
        return false;
      }
