...
export function getSpanCache() {
  return spanCache;
}
export function getSpans(limit?: number) {
  return spanCache.getAllSpans(limit);
}
export function getSpansByTraceId(traceId: string) {
  return spanCache.getSpansByTraceId(traceId);
}
export function getSpansByFunctionName(name: string) {
  return spanCache.getSpansByFunctionName(name);
}
export function getSpansByTimeRange(startTime: number, endTime: number) {
  return spanCache.getSpansByTimeRange(startTime, endTime);
}
export function getTraceIds(
  ...
  return spanCache.getTraceIds(startTime, endTime, limit);
}
export function getSpanStats() {
  return spanCache.getStatistics();
}
...
export function getTracesWithSpans(
  startTime?: number,
  endTime?: number,
  limit?: number,
) {
  ...
  if (startTime !== undefined || endTime !== undefined) {
    filteredSpans = allSpans.filter((span) => {
      ...
      return (
        (startTime === undefined || spanTime >= startTime) &&
...
function wrapFunction<F extends (...args: any[]) => any>(
  fn: F,
  spanName: string,
  type: "user_function" | "class_method" = "user_function",
): F {
 ...
 */
export function wrapUserFunction<T extends (...args: any[]) => any>(
  fn: T,
  name?: string,
): T {
...
function wrapClassPrototype(cls: any, className: string) {
  if (!cls || !cls.prototype) {
    debugLog.log(
      `[DEBUG] wrapClassPrototype: No class or prototype for ${className}`,
    );
    return;
  }
  ...
  if (
    builtinPrototypes.includes(className) ||
    builtinPrototypes.includes(cls.name)
  ) {
    debugLog.log(
      `[DEBUG] wrapClassPrototype: Skipping built-in prototype ${className}`,
    );
    return;
  }

  // Logger public methods whitelist - only wrap these methods, skip internal helper methods like formatMessage
  const loggerPublicMethods = [
    "info",
    "warn",
    "error",
    "debug",
  ...
  const isLoggerClass =
    className.toLowerCase().includes("logger") ||
  ...
  for (const name of names) {
    ...
    if (isLoggerClass && !loggerPublicMethods.includes(name)) {
      debugLog.log(
        `[DEBUG] wrapClassPrototype: Skipping internal method ${className}.${name}`,
      );
      continue;
    }
    ...
    debugLog.log(
      `[DEBUG] wrapClassPrototype: Wrapping method ${name} -> ${spanName}`,
    );
...
function wrapExports(mod: any, modulePath: string): any {
  ...
  // Add wrapping logs
  debugLog.log(
    `[DEBUG] wrapExports called for module: ${modulePath}, base: ${base}`,
  );
  debugLog.log(
    `[DEBUG] Module exports type: ${typeof mod}, keys: ${typeof mod === "object" ? Object.keys(mod) : "N/A"}`,
  );

  if (typeof mod === "function") {
    const isClass = !!(
      mod.prototype && Object.getOwnPropertyNames(mod.prototype).length > 1
    );
    if (isClass) {
  ...
  if (typeof mod === "object") {
    debugLog.log(
      `[DEBUG] Processing object module with ${Object.keys(mod).length} properties`,
    );
    for (const key of Object.keys(mod)) {
      const val = (mod as any)[key];
      if (typeof val === "function") {
        debugLog.log(
          `[DEBUG] Found function: ${key}, type: ${val.name || "anonymous"}, has prototype: ${!!val.prototype}`,
        );
        if (
          val.prototype &&
        ...
        const isLoggerLike = loggerPublicMethods.some(
          (m) => typeof (val as any)[m] === "function",
        );
          ...
          if (typeof objVal === "function") {
            ...
            if (isLoggerLike) {
              if (!loggerPublicMethods.includes(objKey)) {
                debugLog.log(
                  `[DEBUG] Skipping internal method on ${key}: ${objKey}`,
                );
                continue;
              }
            }
            ...
            debugLog.log(
              `[DEBUG] Found method on ${key}: ${objKey}, type: ${objVal.name || "anonymous"}`,
            );
            if (
              objVal.prototype &&
              Object.getOwnPropertyNames(objVal.prototype).length > 1
            ) {
              debugLog.log(
                `[DEBUG] Wrapping class prototype for method: ${key}.${objKey}`,
              );
              wrapClassPrototype(objVal, objVal.name || `${key}.${objKey}`);
            } else {
              debugLog.log(
                `[DEBUG] Wrapping method: ${key}.${objKey} -> ${key}.${objKey}`,
              );
              (val as any)[objKey] = wrapFunction(
                objVal,
                `${key}.${objKey}`,
                "user_function",
              );
        ...
        if (proto && proto !== Object.prototype) {
          debugLog.log(
            `[DEBUG] Checking prototype for ${key}:`,
...
function shouldWrap(resolvedPath: string): boolean {
  ...
  if (
    resolvedPath.includes("instrumentation.ts") ||
...
function wrapRequireCache(): void {
  ...
  debugLog.log(
    `[DEBUG] wrapRequireCache: Found ${Object.keys(cache).length} modules in cache`,
  );
  let wrappedCount = 0;
...
if (!isDevelopment) {
  ...
  debugLog.log(
    `[DEBUG] Intercepted ${consoleMethods.length} console methods: ${consoleMethods.join(", ")}`,
  );
} else {
...
function connectToProxyServer() {
  ...
  const wsUrl =
    process.env.PROXY_WS_URL || "wss://api.syn-cause.com/codeproxy/ws";
  ...
  try {
    ...
    wsConnection.on("close", (code: number, reason: Buffer) => {
      ...
      debugLog.log(
        `[DEBUG] Disconnected from proxy server (${wsUrl}): ${code} - ${reason.toString()}`,
      );
    ...
    debugLog.error(
      `[DEBUG] Failed to connect to proxy server (${wsUrl}):`,
      error,
    );
...
function handleDataRequest(incomingData: any) {
  ...
  try {
    ...
    const requestData =
      incomingData.data && !incomingData.action
        ? incomingData.data
        : incomingData;

    debugLog.log(
      "[DEBUG] Unwrapped request data:",
    ...
    switch (action) {
      case "get_spans":
        const startTime = params.startTime
          ? Number.parseInt(params.startTime)
          : undefined;
        const endTime = params.endTime ? Number.parseInt(params.endTime) : undefined;
        const traceId = params.traceId;
        const functionName = params.functionName;
        const limit = params.limit ? Number.parseInt(params.limit) : undefined;
      ...
      case "get_traces":
        const tracesStartTime = params?.startTime
          ? Number.parseInt(params.startTime)
          : undefined;
        const tracesEndTime = params?.endTime
          ? Number.parseInt(params.endTime)
          : undefined;
        const tracesLimit = params?.limit ? Number.parseInt(params.limit) : undefined;

        const traces = getTracesWithSpans(
          tracesStartTime,
          tracesEndTime,
          tracesLimit,
        );
        ...
        response.data.query = {
          startTime: tracesStartTime,
          endTime: tracesEndTime,
          limit: tracesLimit,
        };
        break;
      ...
      case "get_stats":
        ...
        response.data.stats = {
          ...stats,
          oldestSpan: stats.oldestSpan
            ? new Date(stats.oldestSpan / 1000000).toISOString()
            : null,
          newestSpan: stats.newestSpan
            ? new Date(stats.newestSpan / 1000000).toISOString()
            : null,
...
function startInstrumentationServer() {
  ...
  const port = process.env.INSTRUMENTATION_PORT
    ? Number.parseInt(process.env.INSTRUMENTATION_PORT, 10)
    : 43210;
  const app = express();
  ...
  app.get("/remote-debug/spans", (req: Request, res: Response) => {
    try {
      const startTime = req.query.startTime
        ? Number.parseInt(String(req.query.startTime))
        : undefined;
      const endTime = req.query.endTime
        ? Number.parseInt(String(req.query.endTime))
        : undefined;
      const traceId = req.query.traceId ? String(req.query.traceId) : undefined;
      const functionName = req.query.functionName
        ? String(req.query.functionName)
        : undefined;
      const limit = req.query.limit
        ? Number.parseInt(String(req.query.limit))
        : undefined;
  ...
  app.get("/remote-debug/spans/stats", (_req: Request, res: Response) => {
    try {
      ...
      res.json({
        ...
        metadata: {
          timestamp: new Date().toISOString(),
          endpoint: "/remote-debug/spans/stats",
        },
      });
    } catch (error: any) {
      res
...
export function init() {
  ...
  debugLog.log(
    "[DEBUG] Instrumentation initialized (Loader-based wrapping in production)",
  );
}
