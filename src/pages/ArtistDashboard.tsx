...
const ArtistDashboard = () => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      sessionStorage.setItem(
        `artist_dashboard_scroll_${activeTab}`,
  ...
  return (
    <div className="min-h-screen bg-gray-50/50 dark:bg-background">
          ...
          <div className="mb-8 sm:mb-12 p-5 sm:p-8 rounded-[2rem] bg-gradient-to-br from-red-500/5 via-orange-500/5 to-amber-500/5 border border-red-500/10 shadow-2xl shadow-red-500/5 animate-in fade-in slide-in-from-top-6 duration-700 ease-out relative overflow-hidden group">
            ...
            <div className="flex flex-col md:flex-row items-center md:items-start gap-6 relative z-10">
              ...
              <div className="flex-1 text-center md:text-left space-y-3">
                <h3 className="font-black text-foreground text-xl sm:text-2xl tracking-tight">
                  Complete Your Creative Profile
                </h3>
                <p className="text-sm sm:text-base text-muted-foreground leading-relaxed font-medium">
                  Unlock the full potential of Artswarit. Your professional
                  presence is currently
                  <span className="mx-1.5 px-2 py-0.5 rounded-lg bg-red-500 text-white font-black">
                ...
                <div className="flex flex-wrap justify-center md:justify-start gap-2 pt-1">
                  {missingFields.map((field) => (
                    <span
                      key={field}
                      className="text-[10px] font-black text-red-700 bg-red-500/10 px-3 py-1 rounded-full uppercase tracking-widest border border-red-500/10 hover:bg-red-500/20 transition-colors cursor-default"
                    >
                      {field}
                    </span>
                  ))}
                ...
                {tabs.map((tabItem) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {isDisabled && (
                        <Lock className="h-2 w-2 sm:h-3 sm:w-3 ml-0.5 opacity-50 shrink-0" />
                      )}
                    </TabsTrigger>
                  );
                })}
      ...
      <div
        className={cn(
          activeTab === "messages" && isChatActive ? "hidden md:block" : "",
        )}
      >
        <Footer />
      </div>
