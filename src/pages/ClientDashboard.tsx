...
const ClientDashboard = () => {
  ...
  const [isDeletingProject, setIsDeletingProject] = useState<string | null>(
    null,
  );
  ...
  const fetchProjects = useCallback(async () => {
    ...
    try {
      ...
      const artistIds = projects
        .filter((p) => p.artist_id)
        .map((p) => p.artist_id);
      const artistProfiles: Record<string, any> = {};
      ...
      const transformedProjects: Project[] = projects.map((project: any) => {
        ...
        return {
          ...
          artist:
            ...
            (project.artist_id ? "Artist" : "Unassigned"),
          artistId: project.artist_id || "",
          artistAvatar:
            artistProfile.avatar_url ||
            `https://api.dicebear.com/7.x/avataaars/svg?seed=${project.artist_id || "default"}`,
          ...
          sourceCurrency: project.amount_usd
            ? "USD"
  ...
  useEffect(() => {
    ...
    const restoreState = () => {
      ...
      try {
        const savedUIState = sessionStorage.getItem(
          "client_dashboard_ui_state",
        );
        if (savedUIState) {
  ...
  useEffect(() => {
    const savedScroll = sessionStorage.getItem(
      `client_dashboard_scroll_${selectedTab}`,
    );
    if (savedScroll) {
      // Small delay to allow content to render
      setTimeout(() => {
        window.scrollTo({ top: Number.parseInt(savedScroll), behavior: "smooth" });
      }, 50);
    } else {
      window.scrollTo(0, 0);
    }
  ...
  const handleUnassignArtist = async (projectId: string) => {
    ...
    try {
      ...
      const msg =
        err?.code === "PGRST301"
          ? "Permission denied. You can only edit your own projects."
          : err?.code === "23505"
            ? "Duplicate entry — this artist is already assigned."
  ...
  const handleNotificationClick = (notification: any) => {
    if (notification.type === "message" || notification.type === "project") {
      if (notification.metadata?.projectId) {
        setProjectModalInitialTab(
          notification.type === "message" ? "communication" : "workflow",
        );
    ...
    try {
      ...
      if (updateError) {
        if (updateError.code === "PGRST301")
          throw new Error(
            "Permission denied — you can only confirm your own projects.",
          );
        throw updateError;
      }
  ...
  const handleArtistSelected = async (artistId: string) => {
    ...
    try {
      ...
      toast.success(
        "Artist assigned successfully. Click confirm to send project.",
      );
  ...
  const handleDeleteProject = async (projectId: string) => {
    ...
    try {
      ...
      if (!data || data.length === 0) {
        console.warn(
          "Delete warning: No rows were deleted. Check RLS or Project ID.",
        );
      }
      ...
      toast.error(err.message || "Failed to delete project", {
        description:
          "Please ensure all associated milestones and files are not locked.",
      });
    } finally {
      setButtonLoading((prev) => ({ ...prev, [key]: false }));
    ...
    <div className="flex flex-col min-h-screen bg-gradient-to-br from-blue-50 via-white to-purple-50 dark:from-background dark:via-background dark:to-background">
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 pb-8 sm:pb-12 pt-[calc(6.5rem+var(--safe-top))] sm:pt-[calc(8rem+var(--safe-top))] lg:pt-[calc(9rem+var(--safe-top))]">
        ...
        <div className="mb-4 sm:mb-6 lg:mb-8 animate-fade-in">
          <h1 className="font-heading text-xl sm:text-2xl lg:text-3xl font-black mb-1 sm:mb-2">
            Client Dashboard
          </h1>
          <p className="text-muted-foreground text-xs sm:text-sm lg:text-base">
            Welcome back,{" "}
            <span className="font-black text-foreground">{userName}</span>!
            Manage your projects and discover new artists.
          </p>
        </div>
              ...
              <div className="flex items-start gap-3 flex-1">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-sm text-muted-foreground mt-1">
                    Complete your profile to access all dashboard features. Your
        ...
        >
            ...
            <div className="overflow-x-auto scroll-smooth snap-x snap-mandatory py-2 pb-4">
              <TabsList className="bg-white/80 dark:bg-card/80 backdrop-blur-md inline-flex sm:flex sm:flex-wrap lg:grid lg:grid-cols-5 xl:grid-cols-10 gap-2 p-1.5 rounded-[1.5rem] shadow-xl border border-border/40 min-w-full sm:min-w-0 h-auto min-h-[80px] sm:min-h-0">
                ...
                >
                  ...
                  <span className="font-bold sm:font-medium whitespace-nowrap">
                    Collection
                  </span>
                  {profileIncomplete && <Lock className="h-3 w-3 ml-0.5" />}
                </TabsTrigger>
          ...
          <TabsContent
            value="overview"
            className="space-y-4 sm:space-y-6 lg:space-y-8 animate-fade-in outline-none focus-visible:ring-0"
            forceMount
          >
                    ...
                    >
                      ...
                      <div className="relative z-10">
                        <div className="flex items-center gap-1.5 sm:gap-3 mb-1 sm:mb-3">
                          ...
                          <h3 className="font-semibold text-[10px] sm:text-xs lg:text-sm text-muted-foreground uppercase tracking-tight sm:tracking-wider truncate">
                            Active
                          </h3>
                        </div>
                        <div className="flex items-baseline gap-1 sm:gap-2">
                          ...
                          <span className="hidden xs:inline text-[8px] sm:text-xs text-muted-foreground font-medium">
                            Projects
                          </span>
                        </div>
                      </div>
                        ...
                        {activeProjects.length === 0 ? (
                          <div className="flex flex-col items-center justify-center py-14 px-6 rounded-[2.5rem] border-2 border-dashed border-border/40 bg-muted/20 backdrop-blur-sm">
                            ...
                            <p className="text-sm text-muted-foreground text-center max-w-xs mb-6 font-medium leading-relaxed opacity-70">
                              Bring your vision to life. Create a project and
                              connect with talented artists.
                            </p>
                            <Button
                              onClick={() => setCreateProjectOpen(true)}
                              className="gap-2 h-11 px-7 rounded-2xl font-black shadow-lg shadow-primary/20"
                            >
                          ...
                          activeProjects.slice(0, 3).map((project, index) => (
                            <div
                              key={project.id}
                              className="group bg-white/80 dark:bg-card/80 backdrop-blur-sm p-5 rounded-2xl shadow-sm border border-muted/30 hover:shadow-lg hover:border-primary/30 transition-all duration-300 animate-fade-in"
                            ...
                            >
                              <div className="flex flex-col sm:flex-row justify-between items-start gap-4">
                                <div className="flex-1 min-w-0">
                                  ...
                                  <h3 className="font-bold text-lg group-hover:text-primary transition-colors truncate mb-1">
                                    {project.title}
                                  </h3>
                                ...
                                <div className="flex sm:flex-col gap-2 shrink-0 self-end sm:self-center">
                                  <Button
                                    size="sm"
                                    variant="outline"
                                    className="rounded-xl px-4 hover:bg-primary hover:text-white transition-all shadow-sm"
                                    onClick={() => {
                          ...
                          {recommendedArtists
                            ...
                            .map((artist, index) => (
                              <Link
                                key={artist.id}
                                to={`/artist/${artist.id}`}
                                className="group bg-white/70 dark:bg-card/70 backdrop-blur-sm p-4 rounded-2xl shadow-sm border border-muted/20 hover:shadow-md hover:border-primary/20 transition-all duration-300 animate-fade-in"
                                ...
                                <div className="flex items-center gap-3">
                                  <div className="relative shrink-0">
                                    <img
                                      ...
                                      alt={artist.name}
                                      className="h-12 w-12 rounded-2xl object-cover ring-2 ring-white dark:ring-border group-hover:ring-primary/50 transition-all"
                                    />
                            ...
                            {notifications.length === 0 ? (
                              ...
                              notifications
                                ...
                                .map((notification, index) => (
                                  ...
                                  >
                                    {!notification.read && (
                                      <div className="mt-1.5 h-2 w-2 rounded-full bg-blue-500 shadow-[0_0_8px_rgba(59,130,246,0.5)] shrink-0" />
                                    )}
                                    <div className="flex-1 min-w-0">
          ...
          <TabsContent
            value="collection"
            className="animate-fade-in outline-none focus-visible:ring-0"
            forceMount
          >
              ...
              {visitedTabs.has("projects") && (
                <>
                  <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 mb-4">
                    <h2 className="font-heading text-base sm:text-lg lg:text-xl font-black uppercase tracking-tight">
                      All Projects
                    </h2>
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
                    <div className="bg-white/60 dark:bg-card/60 backdrop-blur-sm p-4 sm:p-6 rounded-lg sm:rounded-xl shadow-sm border border-blue-100 dark:border-border">
                      <h3 className="font-heading text-base sm:text-lg font-semibold mb-3 sm:mb-4 flex items-center">
                        ...
                        {projectSearch && (
                          <span className="ml-2 text-xs font-normal text-muted-foreground">
                            ({searchedActiveProjects.length} result
                            {searchedActiveProjects.length !== 1 ? "s" : ""})
                          </span>
                        )}
                      </h3>
                      ...
                      <div className="space-y-3 sm:space-y-4">
                        {searchedActiveProjects.length === 0 ? (
                          <div className="flex flex-col items-center justify-center py-12 px-4 rounded-[2rem] border-2 border-dashed border-border/30 bg-muted/10">
                            ...
                            {!projectSearch && (
                              <Button
                                size="sm"
                                onClick={() => setCreateProjectOpen(true)}
                                className="h-9 px-5 rounded-xl font-black text-xs shadow-sm shadow-primary/20"
                              >
                          ...
                          <>
                            {searchedActiveProjects
                              .slice(0, visibleActive)
                              .map((project, index) => (
                                <div
                                  key={project.id}
                    ...
                    <div className="bg-white/60 dark:bg-card/60 backdrop-blur-sm p-4 sm:p-6 rounded-lg sm:rounded-xl shadow-sm border border-blue-100 dark:border-border">
                      <h3 className="font-heading text-base sm:text-lg font-semibold mb-3 sm:mb-4 flex items-center">
                        ...
                        {projectSearch && (
                          <span className="ml-2 text-xs font-normal text-muted-foreground">
                            ({searchedCompletedProjects.length} result
                            {searchedCompletedProjects.length !== 1 ? "s" : ""})
                          </span>
                        )}
                      </h3>
                      <div className="space-y-3 sm:space-y-4">
                        {searchedCompletedProjects.length === 0 ? (
                          ...
                          <>
                            ...
                            {/* Load More — Completed */}
                            {visibleCompleted <
                              searchedCompletedProjects.length && (
                              <div className="flex justify-center pt-2">
                                <Button
                                  variant="outline"
                                  size="sm"
      ...
      <AlertDialog
        ...
        onOpenChange={(open) => !open && setIsDeletingProject(null)}
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              draft project and all its associated data from our servers.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel
              disabled={
                isDeletingProject
                  ? !!buttonLoading[`delete-${isDeletingProject}`]
                  : false
              }
            >
              Cancel
            ...
            >
              {isDeletingProject && buttonLoading[`delete-${isDeletingProject}`]
                ? "Deleting..."
                : "Delete Project"}
            </AlertDialogAction>
          </AlertDialogFooter>
