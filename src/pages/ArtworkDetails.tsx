...
export default function ArtworkDetails({
  ...
  useEffect(() => {
    async function init() {
      ...
      const viewData: { artwork_id: string; user_id?: string } = {
        artwork_id: id,
      };
      ...
      if (meta.admin_banned) {
        ...
        toast({
          ...
          description:
            "This artwork has been removed for violating community guidelines.",
          variant: "destructive",
        });
        return;
      }
    ...
    >
      ...
      <main
        className={cn(
          ...
          isModal
            ? "pt-[var(--safe-top)]"
            : "pt-[calc(var(--navbar-height-mobile)+var(--safe-top)+1rem)] sm:pt-[calc(var(--navbar-height-desktop)+var(--safe-top)+1.5rem)]",
        )}
      >
        <div
          className={cn(
            "max-w-6xl mx-auto px-0",
            !isModal && "sm:px-4 lg:px-8",
          )}
        >
          <div
            className={cn(
              ...
              !isModal
                ? "sm:rounded-2xl border-x-0 sm:border border-border/40 shadow-sm"
                : "border-none",
            )}
          >
            ...
            <div className="flex items-center gap-3 px-3 sm:px-4 py-3">
              <button
                onClick={() => navigate(-1)}
                className="mr-1 text-muted-foreground hover:text-foreground transition-colors sm:hidden"
              >
            ...
            >
                  ...
                  <Dialog>
                    ...
                    <DialogContent className="max-w-[98vw] max-h-[98vh] p-0 border-none bg-background rounded-2xl overflow-hidden flex items-center justify-center">
                      <DialogTitle className="sr-only">
                        Fullscreen — {artwork.title}
                      </DialogTitle>
                      <DialogDescription className="sr-only">
                        Full size view of {artwork.title}
                      </DialogDescription>
                      <img
                        src={artwork.imageUrl}
                        alt={artwork.title}
                        className="max-w-full max-h-[96vh] object-contain"
                      />
                    </DialogContent>
                  </Dialog>
                </>
              ...
              {doubleTapLike && (
                <div className="absolute inset-0 flex items-center justify-center pointer-events-none z-10">
                  <Heart
                    className="h-24 w-24 text-white fill-white drop-shadow-2xl animate-ping"
                    style={{
            ...
            <div className="px-3 sm:px-4 pt-1.5 pb-1 space-y-1">
              <p className="text-sm">
                <Link
                  to={`/artist/${artwork.artistId}`}
                  className="font-semibold text-foreground hover:text-muted-foreground transition-colors mr-1.5"
                >
                  {artwork.artist}
                </Link>
