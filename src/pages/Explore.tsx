...
const Explore = () => {
  const { artworks, loading, error, hasMore, loadMore, loadingMore } =
    usePublicArtworks();
  ...
  const handleFiltersChange = (filters: {
    ...
    if (filters.artworkType && filters.artworkType !== "all") {
      filtered = filtered.filter(
        (artwork) => artwork.type === filters.artworkType,
      );
    }
    ...
    if (filters.approvalStatus && filters.approvalStatus !== "all") {
      filtered = filtered.filter(
        (artwork) =>
          (artwork.approval_status || "").toLowerCase() ===
    ...
    if (typeof filters.minLikes === "number" && filters.minLikes > 0) {
      filtered = filtered.filter(
        (artwork) => (artwork.likes || 0) >= filters.minLikes,
      );
    }
    ...
    if (typeof filters.minViews === "number" && filters.minViews > 0) {
      filtered = filtered.filter(
        (artwork) => (artwork.views || 0) >= filters.minViews,
      );
    }
        ...
        <div className="flex-1 flex items-center justify-center p-4">
          <GlassCard className="p-8 max-w-md w-full text-center space-y-6">
            ...
            <p className="text-muted-foreground font-medium">
              We're having trouble reaching the gallery. Please check your
      ...
      <section className="relative pt-[calc(6rem+var(--safe-top))] sm:pt-[calc(8rem+var(--safe-top))] pb-12 sm:pb-20 overflow-hidden">
        ...
        <div className="max-w-[1400px] relative mx-auto px-4 text-center">
          ...
          <p className="text-base sm:text-lg lg:text-xl text-muted-foreground max-w-2xl mx-auto px-4 font-medium leading-relaxed opacity-80 animate-in fade-in slide-in-from-bottom-8 duration-1000 delay-200">
            Curated masterpieces from visionaries worldwide. Filter by medium,
            style, or artist to find your next obsession.
          </p>
        </div>
      </section>
      ...
      {(trendingArtworks.length > 0 || loading) && (
        <section className="max-w-[1400px] mx-auto px-4 py-8 animate-in fade-in duration-1000 delay-500">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 sm:gap-8">
            {loading
              ? [...Array(4)].map((_, idx) => (
                  <ArtworkSkeleton key={`trend-skeleton-${idx}`} />
                ))
          ...
          {(filteredArtworks && filteredArtworks.length > 0) || loading ? (
            <div className="space-y-12">
              ...
              >
                {loading
                  ? [...Array(8)].map((_, idx) => (
                      <ArtworkSkeleton key={`skeleton-${idx}`} />
                    ))
            ...
            <div className="text-center py-24 sm:py-32">
              <div className="max-w-md mx-auto space-y-8">
                <div className="relative inline-block">
                  ...
                  <div className="relative text-7xl sm:text-8xl animate-bounce">
                    🎨
                  </div>
                </div>
