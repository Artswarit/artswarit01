...
export default function ArtistProfile() {
      ...
      try {
        ...
        const [
          ...
          supabase
            ...
            .maybeSingle(),
          artworksQuery,
          supabase.from("follows").select("id").eq("following_id", id),
          supabase
        ...
        if (artworkIds.length > 0) {
          ...
          (allLikes || []).forEach((like) => {
            if (like.artwork_id) {
              likesPerArtwork[like.artwork_id] =
                (likesPerArtwork[like.artwork_id] || 0) + 1;
            }
          });
        }
        ...
        if (artworkIds.length > 0) {
          ...
          (allViews || []).forEach((view) => {
            if (view.artwork_id) {
              viewsPerArtwork[view.artwork_id] =
                (viewsPerArtwork[view.artwork_id] || 0) + 1;
            }
          });
        }
  ...
  useEffect(() => {
    const loadSettings = async () => {
      ...
      try {
        ...
        /* RLS may block non-auth reads — fall through */
      }
      ...
      type ProfileRow =
        import("@/integrations/supabase/types").Tables<"profiles">;
      ...
      setArtistSettings({
        profileVisibility:
          profileData?.profile_visibility ?? saved.profileVisibility ?? true,
        allowDirectMessages: saved.allowDirectMessages ?? true,
        vacationMode: (profileData?.is_on_vacation ??
          saved.vacationMode ??
          false) as boolean,
        ...
        <div className="flex-1 flex items-center justify-center p-4">
          <GlassCard className="p-8 text-center">
            <p className="text-lg font-bold">
              This artist profile is currently not publicly visible.
            </p>
  ...
  useEffect(() => {
    async function fetchFollowersData() {
      ...
      if (!id || id === ARTIST_UUID_1 || id === ARTIST_UUID_2) {
        console.log(
          "[ARTIST PROFILE] Demo/fallback profile - skipping real DB fetch",
        );
        return;
      }
          ...
          async (payload) => {
            ...
            if (user?.id) {
              ...
              if (
                ...
                newRecord?.follower_id === user.id
              ) {
                setIsFollowing(true);
              } else if (
                payload.eventType === "DELETE" &&
                oldRecord?.follower_id === user.id
              ) {
  ...
  useEffect(() => {
    async function checkSavedStatus() {
      if (!id || !user?.id || id === ARTIST_UUID_1 || id === ARTIST_UUID_2)
        return;
    ...
    if (id === ARTIST_UUID_1 || id === ARTIST_UUID_2) {
      ...
      if (!isFollowing) {
        ...
        toast({
          title: "Followed (Demo)",
          description:
            "You are now following this artist! (Demo artist profile)",
        });
      } else {
        setIsFollowing(false);
    ...
    if (isSaved) {
      ...
      if (!error) {
        ...
        toast({
          variant: "destructive",
          title: "Error",
          description: "Could not unsave artist.",
        });
      }
    } else {
      // Save logic
      ...
      if (!error) {
        ...
        toast({
          variant: "destructive",
          title: "Error",
          description: "Could not save artist.",
        });
      }
    }
    setLoadingSave(false);
      ...
      const { error } = await supabase.from("projects").insert([
        {
          ...
          budget: newProject.budget ? Number.parseFloat(newProject.budget) : null,
  ...
  const handleRequestProject = () => {
    ...
    if (!isArtistOwner && artistSettings.vacationMode === true) {
      toast({
        ...
        description:
          "This artist is currently on vacation and not accepting new requests.",
      });
      return;
    }
  ...
  const portfolio = (profileState.artworks || []).map((a: any, ix: number) => {
    ...
    const artworkPrice =
      a.price ?? (isDemoProfile ? (ix === 0 ? 0 : 499 + 100 * ix) : 0);
    ...
    const hasPrice =
      artworkPrice !== null && artworkPrice !== undefined && artworkPrice > 0;
    const isPremiumArt =
      (accessType === "premium" && hasPrice) ||
      (isDemoProfile && ix === 1 && hasPrice);
    const isExclusiveArt =
      accessType === "exclusive" || (isDemoProfile && ix === 2);
          ...
          artist={{
            ...
            rating:
              artistReviews.length > 0
                ? artistReviews.reduce((sum, r) => sum + r.rating, 0) /
          ...
          */}
          {(() => {
            ...
            const scheduledVac =
              !showBusy &&
              nextVacationStart &&
              new Date(nextVacationStart) > new Date();
            return (
              <div
              ...
              >
                <div className="flex items-center gap-2 sm:gap-3">
                  <span
                    className={`grid place-items-center rounded-lg p-2 ${showBusy ? "bg-amber-100/60" : "bg-red-100/60"}`}
                  >
                    <Calendar
                      className={`${showBusy ? "text-amber-700" : "text-red-700"} w-4 h-4`}
                      aria-hidden="true"
                    />
                  </span>
                  <span
                    className={`text-xs sm:text-sm font-black uppercase tracking-widest ${accentText}`}
                  >
                    ...
                    <>
                      ...
                      {(nextBusyStart || nextBusyEnd) && (
                        <span className="text-xs sm:text-sm font-semibold text-foreground">
                          {nextBusyStart &&
                            formatDate(new Date(nextBusyStart), "MMM d, yyyy")}
                          {nextBusyStart && nextBusyEnd && " - "}
                          {nextBusyEnd &&
                            formatDate(new Date(nextBusyEnd), "MMM d, yyyy")}
                        </span>
                      )}
                    ...
                    <>
                      ...
                      {(nextVacationStart || nextVacationEnd) && (
                        <span className="text-xs sm:text-sm font-semibold text-foreground">
                          {nextVacationStart &&
                            formatDate(
                              new Date(nextVacationStart),
                              "MMM d, yyyy",
                            )}
                          {nextVacationStart && nextVacationEnd && " - "}
                          {nextVacationEnd &&
                            formatDate(
                              new Date(nextVacationEnd),
                              "MMM d, yyyy",
                            )}
                        </span>
                      )}
          ...
          >
            <div className="flex items-center gap-2 sm:gap-3">
              <span className="grid place-items-center rounded-lg p-2 bg-emerald-100/60">
                <TrendingUp
                  className="text-emerald-700 w-4 h-4"
                  aria-hidden="true"
                />
              </span>
